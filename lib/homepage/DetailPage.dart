...
class _DetailPageState extends State<DetailPage> {
  ...
  Future<void> postComment(String content) async {
    final String uri =
        'http://10.0.2.2:4000/api/v1/trade-board/${widget.tradeId}/comment';
    try {
  ...
  Future<void> fetchComments() async {
    final String uri = 'http://10.0.2.2:4000/api/v1/trade-board/${widget.tradeId}/comment-list';
    try {
      http.Response response = await http.get(Uri.parse(uri), headers: {
  ...
  Future<void> deleteComment(int commentNumber) async {
    final String uri = 'http://10.0.2.2:4000/api/v1/trade-board/${widget.tradeId}/$commentNumber';
    try {
      http.Response response = await http.delete(
        Uri.parse(uri),
        headers: {
  ...
  Future<int?> _getFavoriteCountFromLatestList(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> latestList = responseData['tradeBoardListItemList'];
  ...
  Future<void> toggleFavorite() async {
    ...
    final String uri =
        'http://10.0.2.2:4000/api/v1/trade-board/${widget.tradeId}/favorite';
    try {
      final Map<String, dynamic> requestBody = {
